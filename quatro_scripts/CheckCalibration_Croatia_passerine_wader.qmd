---
title: "Check MR1 calibration spring 2023 Passerine & Wader - prior 7dBm correction"
author: "Baptiste schmid"
format: html
editor: visual
params:
  outdir: "C:/Users/bs/OneDrive - Vogelwarte/31403 Radartechnik/SBRS divers/Calibration_MR1/Croatia_2023_passerine_wader"
execute:
  echo: false
---
...
radarID_siteName_tag <- c("BS223_BS225_BS226_BS227", "HR")
```
...
databases:
  ...
  - dbname: 'HR_Vrana_2023_prior7dBCorrection'
    startTime: '2023-03-01 14:00'
    stopTime: '2023-05-31 00:00'
...
class_sel = c('passerine_type','wader_type')
filtered_data <- echo %>%
  filter(
  ...
  mutate(radar_label =paste0("BS", substr(radarID, 1,3))) 
...
passerine_plot <- ggplot(filtered_data, aes(x = feature14.maxLevel, y = feature1.altitude_AGL)) +
  # Add points FIRST (so they're drawn on the bottom layer)
  geom_point(alpha = 0.2, color = "lightblue", size = 0.5) +
  # Add quantile isolines AFTER points (so they're drawn on top)
  stat_density_2d(geom = "density_2d", 
                  aes(color = after_stat(level)),
                  contour_var = "ndensity",  # Use normalized density
                  breaks = c(0.25, 0.5, 0.75, 0.9),  # Now these are quantiles
                  linewidth = 0.5) +
  scale_color_viridis_c(option = "plasma", name = "Density Quantile"#,
  ...
  labs(
    ...
    x = "Maximum Level (dBm)",
    y = "Height (m)",
    caption = "Note: Isolines represent density quantiles (1%, 5%, 10%, 25%, 50%)"# 99%, 95%, 90%, 75%, 50%
  ) +
  # Theme adjustments
  theme_minimal() +
  theme(
...
file_name <- paste0( "Dbz vs height for ", paste(class_sel, collapse = "-"), " in ", paste(radarID_siteName_tag, collapse = "-"), ".png" )
