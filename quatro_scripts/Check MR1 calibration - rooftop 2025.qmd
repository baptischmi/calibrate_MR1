...
date_daynight_table <- echo %>%
  select(radarID, date_sunset, dayNight) %>%
  count(date_sunset, dayNight, radarID) %>%
...
```{r}
# Find the two dates (date_sunset x dayNight) with maximum counts
max_dates <- echo %>%
  select(date_sunset, dayNight) %>%
  count(date_sunset, dayNight) %>%
  arrange(desc(n)) %>%
  slice_head(n = 2)

# Filter echo data to only include the two dates with maximum counts
echo_filtered <- echo %>%
  semi_join(max_dates, by = c("date_sunset", "dayNight"))
